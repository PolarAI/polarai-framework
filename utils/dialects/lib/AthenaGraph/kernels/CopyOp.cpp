...
void CopyOp::produceKernel(OpBuilder& builder) {
  ...
  auto inp = builder.create<LoadOp>(builder.getUnknownLoc(),
                                  kernel.getArgument(0), idx.getResult());
  builder.create<StoreOp>(builder.getUnknownLoc(), inp, kernel.getArgument(1),
                          idx.getResult());
